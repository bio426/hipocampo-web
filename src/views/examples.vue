<template>
	<div id="examples">
		...
		<section class="indications">
			<div class="container">
				<p>
					Muestras gratis de nuestros libros. /
					<strong>Mostrando {{bookExamples.length}} resultados</strong>
				</p>
			</div>
		</section>
		<div class="container">
			<div
				class="bookExample"
				v-for="(example, index) in bookExamples"
				:key="index"
			>
				<img :src="example.image" alt="portada de ejemplo" />
				<h2>{{example.title}}</h2>
				<p>
					{{example.description}}
				</p>
...
export default {
	...
	setup() {
		...
		let bookExamples = ref([])

		async function getExamples() {
			let snapshot = await db.collection("examples").get()
			snapshot.forEach((doc) => {
				bookExamples.value.push({
					id: doc.id,
					...doc.data(),
				})
			})
...
<style src="@/assets/styles/pages/examples.scss"></style>
